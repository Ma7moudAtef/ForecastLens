...
def test_item_mode_split_121_absolute_127_relative(prep):
    ...
    assert (item_mode == "absolute").sum() == 121
    assert (item_mode == "relative").sum() == 127
...
def test_exactly_one_orphan(prep):
    ...
    assert orphans.iloc[0]["mode"] == "relative"   # permanent, never flips
